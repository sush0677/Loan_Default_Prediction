...
def upload_file():
  ...
  if file:
    ...
    # Display the first 10 to 15 columns of the CSV file
    first_columns = df.iloc[:, :15].to_dict(orient='records')
    ...
    return redirect(url_for('result', filename=filename, first_columns=first_columns))
...
def save_classification_report(cr, filename='classification_report.png', title='Classification Report',
                               cmap=plt.cm.Blues):
  lines = cr.split('\n')
  classes = []
  plotMat = []
